...
pdf_file = st.file_uploader("Ajouter un relevé de compte (PDF)", type=["pdf"], key="julien_pdf_file")
pdf_category = st.selectbox("Catégorie par défaut pour les lignes importées", CATEGORY_OPTIONS, key="julien_pdf_cat")
...
if pdf_file is not None:
    ...
    if parsed_df.empty:
        ...
        selected_comptes = st.multiselect("Comptes détectés à importer", comptes, default=comptes, key="julien_pdf_comptes") if comptes else []
        ...
        if st.button("Importer ces lignes", type="primary", width="stretch", key="julien_pdf_import_btn"):
...
df_all = pd.DataFrame(st.session_state.julien_transactions) if st.session_state.julien_transactions else pd.DataFrame(columns=["Date","Description","Montant","Catégorie"])
if not df_all.empty:
...
selected_month_state = st.session_state.get("ju_month", current_month if current_month in months else months[0])
...
with colf1:
    selected_month = st.selectbox("Mois", options=months, index=month_index, key="ju_month")
with colf2:
    cat_filter = st.selectbox("Catégorie", options=["Toutes"] + CATEGORY_OPTIONS, key="ju_cat")
...
if st.session_state.julien_transactions:
    df = pd.DataFrame(st.session_state.julien_transactions)
    ...
    for idx, trans in enumerate(st.session_state.julien_transactions):
        with st.container():
            ...
            edit_state_key = f"edit_state_julien_{idx}"
            edit_btn_key = f"edit_btn_julien_{idx}"
            delete_btn_key = f"del_julien_{idx}"
            ...
            with c5:
                ...
                if b2.button("🗑️", key=delete_btn_key):
                    st.session_state.julien_transactions.pop(idx)
                    save_transactions("julien", st.session_state.julien_transactions)
        ...
        if st.session_state.get(edit_state_key, False):
            with st.form(key=f"form_{edit_state_key}"):
                ...
                if submitted:
                    st.session_state.julien_transactions[idx] = {
